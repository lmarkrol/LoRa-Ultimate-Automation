...
def lamaWaktuDown(Waktu):
    ...
    timediff = sekarang - waktuDevice #calculate the difference
    seconds = timediff.total_seconds()   #convert to seconds
    #days = timediff.total_days()
    hours = seconds/3600 #Convert seconds to hours (there are 3600 seconds in an hour)
    #days = seconds/86400
    #Show the total
...
def buatExcel(json):
    ...
    for x in range(Gateway):
        ...
        gatewayTrue = "4" in arrGateway         #Buat Lora Indonesia itu 4, opsi lain 3 tapi itu gatau gajelas
        ...
        if(gatewayTrue == True): 
            ...
            #Catatan untuk gateway dengan MAC address 80029cdf35950000, dia gaada message power
...
def buatFolder(file1, file2):
    #buat folder untuk men-save txtFile dan excel file
    ...
    saveDirectory = rootdir + "/" + datetime.datetime.now().strftime('%d-%m-%Y') #INI PERLU DISESUAIKAN DENGAN DIRECTORY PROGRAMNYA
...
def tugasUtama():
        ...
        if(gatewayTrue == True): 
            ...
            while y < len((json).get('gateways')[x].get('modules')): #ada 6 module, ada juga yg 5
                ...
                if( str( (json).get('gateways')[x].get('modules')[y].get('type') ) == "" ):
                    cekPower = False
                    got += 1
                if( (json).get('gateways')[x].get('mac') == '80029cdf35950000' and y == 0 ): #INI PENGECUALIAN AJA, KARNA GAADA MEssage YG TYPENYA "power" atau ""
                    cekPower = False
                    got += 1 #biar gak berulang kali dijalanin, makannya pake y == 0
                if(y == len((json).get('gateways')[x].get('modules')) - 1):
            ...
            """
            if( (json).get('gateways')[x].get('mac') == '80029cdf35950000' ):
                jprint((json).get('gateways')[x].get('modules'))
                print(cekPower, cekMobile, cekEther)
                print("GOT: ", got)
            """
    ...
    if os.path.isfile('dataset.xlsx'):
        ...
        client.send_file(botUsername, rootdir + "/" + TXTfile) #INI PERLU DISESUAIKAN DENGAN DIRECTORY PROGRAMNYA

        #Send to group
        message = 'Laporan pukul ' + laporanPukul + detailOutput + "New Gateway Down: \n" + messageDown + "\n\n" + "New Gateway Up: \n" + messageUp
        send_message(NOCid, token, message)

        #path = 'C:/Users/t/Documents/LoRa Ultimate Automation/' + TXTfile #lokasi txt file detail gateway error berada
        path = rootdir + "/" + TXTfile #lokasi txt file detail gateway error berada
        files = {'document':open(path,'rb')} #membuka sebuah dokumen dari path    

        resp = requests.post('https://api.telegram.org/bot1665551618:AAFvpiCLP19vYNPVez0-wlNFdcEe8HlK2Jw/sendDocument?chat_id=-1001249682496&caption=All_Gateway_Down_Detail_at: {}'.format(laporanPukul), files = files)#, verify=False, timeout = 30)
        client.disconnect()
        print("\nMessage successfully sent to telegram")
        #https://stackoverflow.com/questions/23013220/max-retries-exceeded-with-url-in-requests (masalah gamau send .txt file ke grup)
...
parametersLora = {
    "format":"json",
    "limit":"5000" #limit data yang mau diambil
...
schedule.every().hour.at(":00").do(lambda: tugasUtama())
...
schedule.every().hour.at(":23").do(lambda: reLogin())
